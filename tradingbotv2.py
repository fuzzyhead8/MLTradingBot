...
class EnhancedGoldSentimentTrader(Strategy): 
    def initialize(self, 
        ...
        # Initialize REST API once to avoid creating multiple thread pools
    ...
    def check_volume_confirmation(self):
        ...
        try:
            # Get current volume
            current_bars = self.get_historical_prices(self.symbol, 1, "day")
            ...
            historical_bars = self.get_historical_prices(self.symbol, 20, "day")
    ...
    def check_volatility(self):
        ...
        try:
            ...
            bars = self.get_historical_prices(self.symbol, 14, "day")
...
if __name__ == "__main__":
    ...
    # Test news keywords mode
    if args.mode == 'test_news':
        print("Testing news keyword matching...")
        headlines, stats = test_news_keywords(
            api_key=API_KEY,
            api_secret=API_SECRET,
            base_url=BASE_URL,
        ...
        print("\nTest complete. Found", len(headlines), "relevant headlines.")
        if len(headlines) > 0:
            print("Keywords are working correctly.")
        else:
            print("No relevant headlines found. Consider:")
            print("1. Expanding your keyword list")
            print("2. Increasing the lookback period")
            print("3. Checking if your API keys have news access")
        
    # Live trading mode
    elif args.mode == 'live':
        print("Starting live trading mode...")
        broker = Alpaca(ALPACA_CREDS)
        strategy = EnhancedGoldSentimentTrader(
            name='gold_sentiment_trader', 
            broker=broker,
            parameters={
                "symbol": "GLD",
                "cash_at_risk": 0.5,
                ...
                "stop_loss_pct": 0.03,
                "sentiment_confirmation_count": 2,
                "min_volume_ratio": 1.2
            }
        )
        
        # Set up the trader and run live
        trader = Trader()
        ...
        try:
            ...
            if 'broker' in locals():
                if hasattr(broker, 'api'):
                    if hasattr(broker.api, '_thread_pool'):
                        if not broker.api._thread_pool._shutdown:
                            broker.api._thread_pool.shutdown()